...
#if !defined(RC_CONTROLLER_API_TEMPERATURECONTROLINTERFACE_H)
#define RC_CONTROLLER_API_TEMPERATURECONTROLINTERFACE_H
 ...
 * @brief 恒温操作结果。
 */
enum class RCThermostatOperateResult
{
 ...
 */
class LUIP_SHARE RCTemperatureControlInterface : public DeviceInterface
{
public:
    RCTemperatureControlInterface(DscpAddress addr);
    // 查询温度传感器的校准系数。
    RCTempCalibrateFactor GetCalibrateFactor() const;
    // 设置温度传感器的校准系数。
    bool SetCalibrateFactor(RCTempCalibrateFactor factor);
    // 查询当前温度。
    RCTemperature GetTemperature() const;
    // 查询恒温控制参数。
    RCThermostatParam GetThermostatParam() const;
    // 设置恒温控制参数。
    bool SetThermostatParam(RCThermostatParam param);
    // 查询恒温器的工作状态。
    Uint16 GetThermostatStatus() const;
    // 开始恒温。
    bool StartThermostat(RCThermostatMode mode, float targetTemp, float toleranceTemp, float timeout);
    // 停止恒温控制。
    bool StopThermostat();
    // 设置温度上报周期。
    bool SetTemperatureNotifyPeriod(float period);
    // 恒温结果事件。
    RCThermostatResult ExpectThermostat(long timeout);
    // 注册温度定时上报事件。
    ...
    // 查询当前恒温控制参数。
    RCThermostatParam GetCurrentThermostatParam() const;
    // 设置当前恒温控制参数。
    bool SetCurrentThermostatParam(RCThermostatParam param);
    // 紫外灯开
    bool TurnOnRays();
    // 紫外灯开关
    bool TurnOffRays();
