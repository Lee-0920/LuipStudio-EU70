...
#include "LCTemperatureControlInterface.h"
 ...
 * @param[in] addr 设备地址。
 */
LCTemperatureControlInterface::LCTemperatureControlInterface(DscpAddress addr)
...
LCTempCalibrateFactor LCTemperatureControlInterface::GetCalibrateFactor() const
{
    LCTempCalibrateFactor factor = {0, 0, 0};
...
LCTempCalibrateFactor LCTemperatureControlInterface::GetCalibrateFactorForTOC(Uint8 index) const
{
    LCTempCalibrateFactor factor = {0, 0, 0};
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool LCTemperatureControlInterface::SetCalibrateFactor(LCTempCalibrateFactor factor)
{
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool LCTemperatureControlInterface::SetCalibrateFactorForTOC(Uint8 index, LCTempCalibrateFactor factor)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[13] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
    memcpy(cmdData+1, &factor, sizeof(LCTempCalibrateFactor));
 ...
 */
LCTemperature LCTemperatureControlInterface::GetTemperature() const
{
    LCTemperature temp;
 ...
 */
LCThermostatParam LCTemperatureControlInterface::GetThermostatParam() const
{
    LCThermostatParam param;
    ...
    if (resp)
    {
        param = *((LCThermostatParam*)resp->data);
    }
...
* @return 返回状态，操作是否成功。
 */
bool LCTemperatureControlInterface::SetThermostatParam(LCThermostatParam param)
{
    Uint16 status = DscpStatus::Error;
...
Uint16 LCTemperatureControlInterface::GetThermostatStatus() const
{
...
/**
 * @brief 修改恒温器参数，带索引值,先停止再以新参数重新开始恒温
 ...
 */
bool LCTemperatureControlInterface::StartThermostat(LCThermostatMode mode, float targetTemp, float toleranceTemp, float timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[13] = {0};
 ...
 * @note 该命令将立即返回。
 */
bool LCTemperatureControlInterface::StopThermostat()
{
    Uint16 status = DscpStatus::Error;
 ...
 */
bool LCTemperatureControlInterface::SetTemperatureNotifyPeriod(float period)
{
    Uint16 status = DscpStatus::Error;
 ...
 *  - @ref temp float，当前温度。
 */
LCThermostatResult LCTemperatureControlInterface::ExpectThermostat(long timeout)
{
    LCThermostatResult thermostatResult;
 ...
 */
void LCTemperatureControlInterface::RegisterTemperatureNotice(IEventReceivable *handle)
{
 ...
 */
bool LCTemperatureControlInterface::BoxFanSetOutput(float level)
{
    Uint16 status = DscpStatus::Error;
 ...
 * @param level float ,风扇速率，对应高电平占空比。默认为0，风扇关闭。
 */
bool LCTemperatureControlInterface::BoxFanSetOutputForTOC(Uint8 index, float level)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[5] = {0};

    cmdData[0] = index;
    memcpy(cmdData+1, &level, sizeof(float));
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_TCI_TURN_BOXFAN , cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
 ...
 * @param level float ,风扇速率，对应高电平占空比。默认为0，风扇关闭。
 */
bool LCTemperatureControlInterface::DigestionFanSetOutputForTOC(Uint8 index, float level)
{
    Uint16 status = DscpStatus::Error;
 ...
 * @note
 */
float LCTemperatureControlInterface::GetHeaterMaxDutyCycle() const
{
 ...
 */
bool LCTemperatureControlInterface::SetHeaterMaxDutyCycle(float maxDutyCycle)
{
    Uint16 status = DscpStatus::Error;
 ...
 */
LCThermostatParam LCTemperatureControlInterface::GetCurrentThermostatParam() const
{
    LCThermostatParam param;
    memset(&param, 0, sizeof(param));
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool LCTemperatureControlInterface::SetCurrentThermostatParam(LCThermostatParam param)
{
    Uint16 status = DscpStatus::Error;
 ...
 * @brief 开启继电器 温控板紫外灯
 */
bool LCTemperatureControlInterface::TurnOnRays()
{
    Uint16 status = DscpStatus::Error;
 ...
 * @brief 关闭继电器 温控板紫外灯
 */
bool LCTemperatureControlInterface::TurnOffRays()
{
    Uint16 status = DscpStatus::Error;
 ...
 */
bool LCTemperatureControlInterface::ReviseThermostatTemp(Uint8 index, LCThermostatMode mode, float targetTemp, float toleranceTemp, float timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[14] = {0};
    memcpy(cmdData, &index, sizeof(Uint8));
    ...
    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_TCI_REVISE_THERMOS_TEMP, cmdData, sizeof(cmdData)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
 ...
 * @brief 控制燃烧炉继电器
 */
bool LCTemperatureControlInterface::RelayControlForTOC(Uint8 ret)
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_TCI_TURN_ON_RELAY,  &ret, sizeof(ret)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
