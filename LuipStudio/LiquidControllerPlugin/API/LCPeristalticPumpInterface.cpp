...
#include "LCPeristalticPumpInterface.h"
 ...
 * @param[in] addr 设备地址。
 */
LCPeristalticPumpInterface::LCPeristalticPumpInterface(DscpAddress addr)
 ...
 * @return 总泵数目， Uint16。
 */
int LCPeristalticPumpInterface::GetTotalPumps()
{
    Uint16 pumps = 0;
 ...
 */
float LCPeristalticPumpInterface::GetPumpFactor(int index)
{
    float factor = 0;
 ...
 * @return 返回状态，操作是否成功。
 */
bool LCPeristalticPumpInterface::SetPumpFactor(int index, float factor)
{
 ...
 */
LCMotionParam LCPeristalticPumpInterface::GetMotionParam(int index)
{
    LCMotionParam param;
...
* @return 返回状态，操作是否成功。
 */
bool LCPeristalticPumpInterface::SetMotionParam(int index, LCMotionParam param)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[9];

    cmdData[0] = index;
    memcpy(cmdData+1, &param, sizeof(LCMotionParam));
 ...
 */
LCPumpStatus LCPeristalticPumpInterface::GetPumpStatus(int index)
{
    LCPumpStatus status = LCPumpStatus::Idle;
 ...
 */
bool LCPeristalticPumpInterface::StartPump(int index, LCRollDirection dir, float volume, float seep)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[10] = {0};    
    cmdData[0] = index;
 ...
 * @return 返回状态，操作是否成功。
 */
bool LCPeristalticPumpInterface::StopPump(int index)
{
    Uint16 status = DscpStatus::Error;
 ...
 */
float LCPeristalticPumpInterface::GetPumpVolume(int index)
{
    float volume = 0;
 ...
 *  - @ref Stopped 泵操作被停止
 */
LCPumpResult LCPeristalticPumpInterface::ExpectPumpResult(long timeout)
{
    LCPumpResult pumpResult;
