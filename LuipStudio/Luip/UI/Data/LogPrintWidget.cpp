...
LogPrintWidget::LogPrintWidget(String fileName, QWidget *parent) : QWidget(parent),
    m_fileName(fileName)
{
...
//    m_limitEdit->setEnabled(false);
    ...
    m_resultFiles = ResultManager::Instance()->GetMeasureRecordFile(m_fileName);
...
void LogPrintWidget::SlotPrintButton()
{
...
QString LogPrintWidget::GetPrinterAddress()
{
    LuaEngine* luaEngine = LuaEngine::Instance();
    lua_State * state = luaEngine->GetThreadState();
...
int LogPrintWidget::GetPrinterConnectType()
{
    LuaEngine* luaEngine = LuaEngine::Instance();
    lua_State * state = luaEngine->GetThreadState();
...
QString LogPrintWidget::GetPrintHeadString()
{
    QString headStr;
...
QString LogPrintWidget::GetPrintItemString()
{
...
void LogPrintWidget::SlotConnectButton()
{
    if(m_printer->IsConnected())
    {
        m_printer->Close();
...
void LogPrintWidget::SlotCloseButton()
{
    m_printer->Close();
    m_connectButton->setText(tr("未连接"));
    this->hide();
...
void LogPrintWidget::SlotHeadButton()
{
    if(!m_printer->IsConnected())
    {
        MessageDialog msg(tr("打印机未连接!\n"), this,MsgStyle::ONLYOK);
...
void LogPrintWidget::SlotCutButton()
{
    if(!m_printer->IsConnected())
    {
        MessageDialog msg(tr("打印机未连接!\n"), this,MsgStyle::ONLYOK);
...
void LogPrintWidget::SlotBeginTimeCheck(QDateTime time)
{
...
void LogPrintWidget::SlotEndTimeCheck(QDateTime time)
{
...
void LogPrintWidget::SlotPrintLimitCheck(QString str)
{
    if(str.toInt() > 1000)
    {
...
void LogPrintWidget::Show(QWidget *parent, int roleType)
{
    (void)roleType;
...
void LogPrintWidget::showEvent(QShowEvent *event)
{
    if(!m_printer->IsConnected())
    {
...
void LogPrintWidget::paintEvent(QPaintEvent *event)
{
    int height = 100;
...
void LogPrintWidget::OnUpdateWidget(UpdateEvent event, System::String message)
{
    (void)message;    
}
